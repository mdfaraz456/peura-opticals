...
$(document).ready(function () {
    // Toggle the .show class on click of .canvas-has-submenu
    $('.canvas-has-submenu').on('click', function (e) {
        e.preventDefault();  // Prevent default action of the <a> tag
        e.stopPropagation(); // Prevent click from propagating to the document

        // Toggle the 'show' class on the submenu
        $(this).siblings('.canvas-submenu').toggleClass('show');
    });

    // Remove .show class when clicking anywhere else
    $(document).on('click', function () {
        $('.canvas-submenu').removeClass('show');
    });

    // Prevent closing the submenu if clicked inside the submenu
    $('.canvas-submenu').on('click', function (e) {
...
(function ($) {
    ...
    mirandaDoc = {
        ...
        mianMenu() {
            ...
            navMenu.find('li .dd-trigger').on('click', function (e) {
                ...
                // If the li has .has-submenu-2 class
                if (parentLi.hasClass('has-submenu-2')) {
                    // Show or hide the submenu-2
                    if (parentLi.hasClass('active')) {
                        parentLi.children('.submenu-2').stop(true, true).slideDown(350);
                    } else {
                        parentLi.children('.submenu-2').stop(true, true).slideUp(350);
                    }
                } 
                // If the li has .has-submenu-3 class
                else if (parentLi.hasClass('has-submenu-3')) {
                    // Show or hide the submenu-3
                    if (parentLi.hasClass('active')) {
                        parentLi.children('.submenu-3').stop(true, true).slideDown(350);
                    } else {
                        parentLi.children('.submenu-3').stop(true, true).slideUp(350);
                    }
                } 
                // For regular dropdowns (not submenu-2 or submenu-3)
                else {
                    parentLi.children('ul').stop(true, true).slideToggle(350);
                }
            });
        ...
        //===== 06. New Arrival Slider
        newArrivalslider() {
            var tslider = $('.new-arrival-slider');
            var arrows = $('.shop-post-arrow');
            tslider.slick({
                slidesToShow: 4,
                slidesToScroll: 1,
    ...
    $(window).on('load', function () {
        ...
        setTimeout(function () {
            $("#monthlyCrystalModal").modal('show');
            console.log("modal run")
        }, 8000);
  ...
  -------------------------------------------------------------------------------*/
    ...
    // document.querySelector('.image-box').addEventListener('mousemove', function (e) {
    ...
    //         // Move the magnifier
    //         magnifier.style.left = `${posX - magnifier.offsetWidth / 2}px`;
    //         magnifier.style.top = `${posY - magnifier.offsetHeight / 2}px`;

    //         // Set the magnifier background image and position
    //         magnifier.style.backgroundImage = `url(${sliderImage.src})`;
    //         magnifier.style.backgroundPosition = `-${posX * 2}px -${posY * 2}px`;
    //     } else {
