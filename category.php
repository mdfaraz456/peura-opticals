...
<body>
    ...
    <div class="col-md-12">
        <nav aria-label="Product Pagination">
            <ul class="pagination style-1" id="pagination-list">
...
<script>
    ...
    var products = <?php echo json_encode($query); ?>; // Fetch all products from PHP to JS
    ...
    function renderPagination() {
        ...
        paginationHtml += '<li class="page-item"><a class="page-link prev" href="javascript:void(0);">Previous</a></li>';
        ...
        // Render next button
        paginationHtml += '<li class="page-item"><a class="page-link next" href="javascript:void(0);">Next</a></li>';
    ...
    function renderProducts(pageNumber) {
        ...
        productsToDisplay.forEach(function(product) {
            var discountInfo = calculateDiscount(product.price, product.discount);
            var productTypeIds = product.product_type_ids.split(',');
            var productType = productTypeIds.join(', ');

            productsHtml += `
                <div class="col-6 col-xl-3 col-lg-4 col-md-4 col-sm-6 m-md-b15 m-b30" data-product-type="${productType}">
                    <div class="shop-card style-1">
                        <div class="dz-media" id="dz-img">
                            <img class="img-dz" src="adminuploads/products/${product.image}" alt="image">
                            <div class="shop-meta">
                                <a href="product-detail.php?id=${btoa(product.product_id)}" class="btn btn-secondary btn-md btn-rounded">
                                    <i class="fa-solid fa-eye d-md-none d-block"></i>
                                    <span class="d-md-block d-none">View Details</span>
                                </a>
                                <div class="btn btn-primary meta-icon dz-wishicon" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                ...
                                <a href="#" class="btn btn-primary meta-icon dz-carticon cartBuy" data-product-id="${product.product_id}">
                                    <i class="flaticon flaticon-basket"></i>
                                    <i class="flaticon flaticon-basket-on dz-heart-fill"></i>
                                </a>
                            </div>
                        ...
                        <div class="dz-content">
                            <h5 class="title"><a href="product-detail.php?id=${btoa(product.product_id)}">${product.name}</a></h5>
                            <h5 class="price">₹${Number(discountInfo.discountedPrice).toLocaleString()}</h5>
                        </div>
                        <div class="product-tag">
                            <span class="badge">Try On</span>
                        </div>
        ...
        $('.tab-content .shop- .row').html(productsHtml);
    }
    ...
    function updateActivePage() {
        ...
        $('#pagination-list .page-link[data-page="' + currentPage + '"]').parent().addClass('active');
    }
